...
describes what users get. Pick a format with `-o`, or let vndirect choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
vndirect <command> -o table   # aligned columns for reading
vndirect <command> -o jsonl   # one JSON object per line, for piping
vndirect <command> -o json    # a single JSON array
vndirect <command> -o csv     # spreadsheet friendly
vndirect <command> -o tsv     # tab-separated
vndirect <command> -o url     # just the URL column
vndirect <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
vndirect <command> --fields id,title,url
```
...
```bash
vndirect <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
vndirect <command>            # a table, because this is a terminal
vndirect <command> | wc -l    # JSONL, because this is a pipe
```
